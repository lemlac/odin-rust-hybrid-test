...
cargo build --manifest-path="mymath/Cargo.toml"
copy ".\mymath\target\debug\mymath.dll" "%OUT_DIR%\"
...
odin build source ^
    -collection:mymath=mymath\target\debug ^
    -strict-style -vet -debug -define:RAYLIB_SHARED=true ^
    -build-mode:dll -out:%OUT_DIR%/game.dll ^
    -pdb-name:%GAME_PDBS_DIR%\game_%PDB_NUMBER%.pdb
IF %ERRORLEVEL% NEQ 0 (
    echo Odin build failed. Check the errors above.
    exit /b 1
)
